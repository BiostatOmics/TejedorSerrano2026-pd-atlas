# Description: Generate Figure 2 panels: forest+tree plots (a-e) and multi-covariate heatmap (f-j)
...
# %% Figure 2, panels f-j: multi-covariate effect heatmap
{
  ...
  # Build complete parent-child maps for the cell type hierarchy
  ...
  # Traverse ancestor hierarchy for a given node (DFS)
  ...
  get_bar_df <- function(heatmap_df, hierarchy_map) {
    ...
    # Count how many children each node has (only among subtypes present in this region)
    ...
    # Propagate maximum credibility from each node's family
